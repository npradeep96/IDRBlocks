...
import sys
sys.path.append('../')
import utils.class_sequence as cs
import numpy as np
import csv
...
def generate_data_matrix(list_of_files, label, write_file_path, custom_aa_list=False, aa_group=None, aa_group_names=None, coarse_graining=10):
    ...
    for filename in list_of_files:
        ...
        outfile_name.append(dirName + '/' + filename_stripped.rstrip('.csv') + '-features.csv')
        ...
        with open(filename) as csv_file:
            ...
            for row in csv_reader:
                ...
                if first_line:
                    ...
                    all_seqs_list.append(sequence_id)

                    list_of_features = list()
                    list_of_features.append(sequence_id)

                    my_seq = cs.sequence(str(sequence_seq), custom_aa_list=custom_aa_list,aa_group=aa_group,aa_group_names=aa_group_names)

                    my_seq.get_composition()
                    list_of_features += my_seq.composition

                    my_seq.smear_sequence(coarse_graining=5, window_length=10)
                    ...
                    # list_of_features += [len(sequence_seq)]

                    list_of_features += [label[i]]
...
def plot_enrichment_vs_cluster_size(write_file_path, output_filename, data_matrix, labels, label_legends, colors,  xlim, ylim, switch, flag_save_files, num_components, num_clusters):
    ...
    # Labels Cutoff at 99% confidence interval
    for i in set(labels):
        if i>=0:
            frac[label_legends[i]] = float(label_counts[i])/sum(label_counts)
            plt.plot(x,1+2.57*np.sqrt(np.divide((1.0-frac[label_legends[i]]),(frac[label_legends[i]]*grid))),colors[i])
            plt.plot(x,1-2.57*np.sqrt(np.divide((1.0-frac[label_legends[i]]),(frac[label_legends[i]]*grid))),colors[i])
